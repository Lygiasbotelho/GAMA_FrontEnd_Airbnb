...
fetch(API_URL)//XMLHttpRequest(API_URI)
  ...
  .then(result => {
    ...
    data.map(quarto => {//dados.map(element => { /*Poderia ter usado o foreach*/
      // Desestruture seu objeto, trazendo os seguintes dados:
      // Foto, tipo, nome e preço da propriedade.
      const { photo, property_type, name, price } = quarto;// const { variavel, chave } = data;

      // Pegue a linha do container onde serão exibidos os cards.
      row = document.querySelector(".row");//row = document.pega("linha");

      // Agora, crie um elemento "div" com a class "col-md-4" do Bootstrap.
      div = document.createElement("div");//div = document.criaElemento("div");
      div.className = "col-md-4";//div.nomeDaClasse = "col-md-4";
